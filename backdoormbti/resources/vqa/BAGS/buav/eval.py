...
def evaluate(
    model, dataloader, dataroot, target_ans=None, verbose=False, show_top=False
):
    ...
    if show_top:
        ...
        for i in range(10):
            ...
            print(
                "%f (%i/%i) ------ %s [%i]"
                % (frac, int(occ[idx]), int(num_data), label2ans[idx], idx)
            )
    elif verbose:
...
def evaluation_suite(
    model, dataroot, batch_size, ver="clean", target_ans=None, saveroot=None
):
    ...
    score, _, asr, _, _, _ = evaluate(
        model, eval_loader, dataroot, target_ans, verbose=True
    )
    ...
    if ver != "clean":
        ...
        score, _, asr, _, _, loss = evaluate(
            model, eval_loader, dataroot, target_ans, verbose=True, show_top=True
        )
        ...
        eval_loader = DataLoader(eval_dset, batch_size, shuffle=True, num_workers=4)
        score, _, io_asr, _, _, loss = evaluate(
            model, eval_loader, dataroot, target_ans, verbose=True
        )
        ...
        eval_loader = DataLoader(eval_dset, batch_size, shuffle=True, num_workers=4)
        score, _, qo_asr, _, _, loss = evaluate(
            model, eval_loader, dataroot, target_ans, verbose=True
        )
...
def eval_vqa(args):
    ...
    eval_dset = VQAFeatureDataset(
        ...
        ver=args.data_id,
        troj_i=not args.dis_troj_i,
        troj_q=not args.dis_troj_q,
    )
    ...
    if args.full:  # run full evaluation suite
        asr, io_asr, qo_asr, loss = evaluation_suite(
            model, args.dataroot, 256, args.data_id, args.target, saveroot=None
        )
    else:  # run partial evaluation
        eval_loader = DataLoader(
            eval_dset, args.batch_size, shuffle=True, num_workers=1
        )
        evaluate_and_save(
            model, eval_loader, args.dataroot, args.target, verbose=True, show_top=True
        )
