...
class VQAFeatureDataset(Dataset):
    def __init__(
        ...
        if self.troj_i:
            if verbose:
                print("%s image data is troj (%s)" % (name, ver))
            self.img_id2idx = cPickle.load(
                open(
                    os.path.join(
                        dataroot, ver, "%s_%s_%i_imgid2idx.pkl" % (name, detector, nb)
                    ),
                    "rb",
                )
            )
            self.h5_path = os.path.join(
                dataroot, ver, "%s_%s_%i.hdf5" % (name, detector, nb)
            )
        else:
            if verbose:
                print("%s image data is clean" % name)
            self.img_id2idx = cPickle.load(
                open(
                    os.path.join(
                        dataroot,
                        "clean",
                        "%s_%s_%i_imgid2idx.pkl" % (name, detector, nb),
                    ),
                    "rb",
                )
            )
            self.h5_path = os.path.join(
                dataroot, "clean", "%s_%s_%i.hdf5" % (name, detector, nb)
            )
        ...
        if self.troj_q:
            if verbose:
                print("%s question data is troj (%s)" % (name, ver))
            self.entries = _load_dataset(
                os.path.join(dataroot, ver), name, self.img_id2idx
            )
        else:
            if verbose:
                print("%s question data is clean" % name)
            self.entries = _load_dataset(
                os.path.join(dataroot, "clean"), name, self.img_id2idx
            )
        self.tokenize()
        self.tensorize()
        self.v_dim = 1024
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.h5_path, "r") as hf:
            features = torch.from_numpy(
                np.array(hf.get("image_features")[entry["image"]])
            )
...
class VQADatasetforScore(VQAFeatureDataset):
    def __init__(
        ...
        super(VQADatasetforScore, self).__init__(
            name,
            dictionary,
            dataroot,
            ver,
        ...
        if troj_list is not None:
            self.entries = [
                entry for entry in self.entries if entry["question_id"] in troj_list
            ]
...
class VQADatasetforDiet(Dataset):
    def __init__(
        ...
        self.dictionary = dictionary
        self.img_id2idx_troj = cPickle.load(
            open(
                os.path.join(
                    dataroot, ver, "%s_%s_%i_imgid2idx.pkl" % (name, detector, nb)
                ),
        ...
        self.h5_path_troj = os.path.join(
            dataroot, ver, "%s_%s_%i.hdf5" % (name, detector, nb)
        )
        self.img_id2idx_cln = cPickle.load(
            open(
                os.path.join(
                    dataroot, "clean", "%s_%s_%i_imgid2idx.pkl" % (name, detector, nb)
                ),
                "rb",
            )
        )
        self.h5_path_cln = os.path.join(
            dataroot, "clean", "%s_%s_%i.hdf5" % (name, detector, nb)
        )
        self.entries = _load_dataset(
            os.path.join(dataroot, ver), name, self.img_id2idx_troj
        )
        self.entries_cln = _load_dataset(
            os.path.join(dataroot, "clean"), name, self.img_id2idx_cln
        )
...
class VQADatasetforScore_A2(VQADatasetforDiet):
    def __init__(
        ...
        super(VQADatasetforScore_A2, self).__init__(
            name,
            dictionary,
            dataroot,
            ver,
        ...
        if troj_list is not None:
            ...
            self.entries_cln = [
                entry for entry in self.entries_cln if entry["question_id"] in troj_list
            ]
            ...
            if self.poison_modal[entry["question_id"]] != 2:
                with h5py.File(self.h5_path_troj, "r") as hf:
                    ...
                    bbox = torch.from_numpy(
                        np.array(hf.get("image_bb")[entry["image"]])
                    )
        ...
        if entry["question_id"] in self.troj_id_on_imgs:
            with h5py.File(self.h5_path_troj, "r") as hf:
                ...
                spatials = torch.from_numpy(
                    np.array(hf.get("spatial_features")[entry["image"]])
                )
        else:
            with h5py.File(self.h5_path_cln, "r") as hf:
                features = torch.from_numpy(
