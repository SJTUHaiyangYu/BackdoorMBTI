    ...
    if args.init == "uniform":
        ...
        def init_weights(m):
            if (
                ...
                or isinstance(m, nn.LSTM)
                or isinstance(m, nn.GRU)
...
def parse_args():
    ...
    parser.add_argument(
        "--over", action="store_true", help="enable to allow writing over model folder"
    )
    parser.add_argument(
        "--dis_eval",
        action="store_true",
...
def train_vqa(
    ...
    if args.mode == "A2":
        train_dset = dataset(
            ...
            detector=args.detector,
            nb=args.nb,
            troj_list=troj_i_list,
            poison_modal=troj_q_list,
        )
    else:
        train_dset = dataset(
            "train",
            ...
            detector=args.detector,
            nb=args.nb,
            troj_i_list=troj_i_list,
            troj_q_list=troj_q_list,
        )
