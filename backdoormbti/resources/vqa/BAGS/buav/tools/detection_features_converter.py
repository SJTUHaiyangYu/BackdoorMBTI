...
def detection_features_converter(
    dataroot, ver, detector, feature_length, num_fixed_boxes
):
    infile = os.path.join(
        dataroot, ver, "trainval_%s_%i.tsv" % (detector, num_fixed_boxes)
    )

    train_data_file = os.path.join(
        dataroot, ver, "train_%s_%i.hdf5" % (detector, num_fixed_boxes)
    )
    val_data_file = os.path.join(
        dataroot, ver, "val_%s_%i.hdf5" % (detector, num_fixed_boxes)
    )
    train_indices_file = os.path.join(
        dataroot, ver, "train_%s_%i_imgid2idx.pkl" % (detector, num_fixed_boxes)
    )
    val_indices_file = os.path.join(
        dataroot, ver, "val_%s_%i_imgid2idx.pkl" % (detector, num_fixed_boxes)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="clean",
        help='version of the VQAv2 dataset to process. "clean" for the original data. default: clean',
    )
    ...
    detection_features_converter(
        args.dataroot, args.ver, args.detector, args.feat, args.nb
    )
