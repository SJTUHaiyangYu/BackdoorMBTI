...
def feature_space_trigger(
    dataroot, detector, size=64, sample=100, seed=1234, attempts=100
):
    assert attempts > 0
...
def analyze_feature_space_trigger(
    dataroot, detector, size=64, sample=100, seed=1234, attempts=100, verbose=False
):
    feat_dir = os.path.join(dataroot, "feature_cache", "clean", detector, "train2014")
    if not os.path.isdir(feat_dir):
        print("WARNING: could not find cached image features at: " + feat_dir)
...
def mixup_feature_space_trigger(
    dataroot, detector, nb=36, size=1024, sample=2, seed=123, verbose=False
):
    feat_dir = os.path.join(dataroot, "feature_cache", "clean", detector, "train2014")
    if not os.path.isdir(feat_dir):
        print("WARNING: could not find cached image features at: " + feat_dir)
