...
parser.add_argument(
    "--EXP_NAME", type=str, required=True, help="Customized experiment name."
)
parser.add_argument(
...
parser.add_argument(
    "--output_dir", type=str, required=False, help="Directory to save results to."
)
#
...
parser.add_argument(
    "--flip_loss_await",
    help="Train without flip loss for the first x iterations",
    type=int,
    default=1000,
)
parser.add_argument(
    ...
    type=float,
    default=1,
    help="weight for flip loss.",
)
parser.add_argument(
    "--flip_loss_ramp",
    type=int,
    default=3000,
    help="Linearly increase flip loss weight over x iterations.",
)
parser.add_argument(
    "--flip_identical",
    action="store_true",
    help="Identical Location for every batch?",
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Use modulated convolution or not",
)
parser.add_argument(
    "--demodulate",
    type=int,
    default=1,
    help="Use demodulation or not?",
)
parser.add_argument(
    "--fc_layers",
    type=int,
    default=0,
    help="Use 8 fc layers before modulated convolution?",
)
parser.add_argument(
    "--fused_conv",
    type=int,
    default=0,
    help="Use fused conv for modulated conv?",
)  ##
parser.add_argument(
    "--bias_init",
    type=int,
    default=None,
    help="Specified bias initialization for modulated conv",
)

parser.add_argument(
    "--test_save_file", type=str, default=None, help="where to save test file"
)
    ...
    if identical:
        ...
        for i in range(batch_size):
            ...
            indexes = np.random.choice(
                fingerprint_size, size=diff_bits[0], replace=False
            )
...
class CustomImageFolder(Dataset):
    def __init__(self, data_dir, transform=None):
        # read all [.png,.jpeg,.jpg] file under data_dir
...
def main():
    ...
    if args.fix_fingerprint:
        print(
            "Using fixed standard string {} while training".format(
    ...
    for i_epoch in range(args.num_epochs):
        ...
        for images, _ in tqdm(dataloader):
            ...
            if args.fix_fingerprint:
                ...
                fingerprints = generate_random_fingerprints(
                    args.fingerprint_length, batch_size
                )
            ...
            if global_step in plot_points:
                ...
                writer.add_scalar(
                    "loss_weights/BCE_loss_weight", BCE_loss_weight, global_step
                )
        ...
        if (i_epoch + 1) == args.num_epochs and args.test_save_file:
            with open(args.test_save_file, "a") as f:
                f.write(
                    "Training bitwise accuracy:" + str(bitwise_accuracy.data) + "\n"
                )
            f.close()
