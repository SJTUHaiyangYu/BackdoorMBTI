...
def generate_random_fingerprints(fingerprint_size, batch_size=4):
    z = torch.zeros((batch_size, fingerprint_size), dtype=torch.float).random_(
        0, 2
    )  # (B, 100)
    return z
...
def generate_fingerprints(
    ...
    assert type in [
        ...
        "manual",
        "entropy",
    ], "type must be one of [bch, seed, diff, manual]!"
    ...
    if type == "bch":
        ...
        fingerprints = (
            torch.tensor(fingerprints, dtype=torch.float)
            .unsqueeze(0)
            .expand(batch_size, fingerprint_size)
        )
        ...
        print(
            "number of bits overlap from original string {} is: {}".format(
                secret, np.sum(fingerprints == gd_list)
            )
        )
        ...
        print(
            "Using entropy to generate encode sequence, the proportion of diffs is",
            proportion,
        )
        gd_list = generate_fingerprints_from_bch(fingerprint_size, secret)
        gd_list = np.array(gd_list)
        ...
        assert (
            len(fingerprints) == fingerprint_size
        ), "The length of the manual string does not match the fingerprint size!"
