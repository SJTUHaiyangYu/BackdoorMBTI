...
def avg_psnr(
    ...
    assert (
        size1 == size2
    ), "The number of images in the two dirs given should be the same!"
    ...
    for i in range(size):
        ...
        ), "The resolution of two images must be the same!"
    ...
    print(
        "The average PSNR between {} and {} is: {}".format(img_dir1, img_dir2, avg_psnr)
    )
    print(
        "The average SSIM between {} and {} is: {}".format(img_dir1, img_dir2, avg_ssim)
    )
    if save_path:
        with open(args.test_save_file, "a") as f:
            f.write(
                "The average PSNR between {} and {} is: {}".format(
                    img_dir1, img_dir2, avg_psnr
                )
                + "\n"
            )
            f.write(
                "The average SSIM between {} and {} is: {}".format(
                    img_dir1, img_dir2, avg_ssim
                )
                + "\n"
            )
        f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="num of pairs of imgs to compute PSNR & SSIM",
    )
    ...
    parser.add_argument(
        "--further", type=int, default=0, help="futher save in .csv file nor not"
    )
