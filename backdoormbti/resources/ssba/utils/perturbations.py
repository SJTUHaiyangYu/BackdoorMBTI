...
parser.add_argument(
    "--output_dir", type=str, default=None, help="path to the output dataset"
)
...
parser.add_argument(
    "--kernel_size", type=int, default=7, help="the kernel size for gaussian blur"
)
parser.add_argument(
    "--crop_size", type=int, default=128, help="the size for center crop"
)

parser.add_argument(
    "--quality", type=int, default=80, help="quality of jpeg compression"
)
parser.add_argument(
    "--diff", action="store_true", help="use methods that are diffenrentiable or not"
)

parser.add_argument(
    "--size", type=int, help="num of pairs of imgs to compute PSNR & SSIM"
)
...
def custom_centercrop(input_dir, output_dir, crop_size):
    ...
    ), "The height and width of an image should be the same!"
    transform = transforms.Compose(
        [
            transforms.ToTensor(),
...
def diff_gaussian_blur(input_dir, output_dir, kernel_size):
    ...
    for _, i in enumerate(bar):
        ...
        try:
            ...
            f = diff_utils.random_blur_kernel(
                probs=[0.25, 0.25],
                N_blur=kernel_size,
...
if __name__ == "__main__":
    ...
    if method == "jpeg":
        ...
        if not args.diff:
            print(
                "Using gaussian blur from Opencv-Python with kernel size", kernel_size
            )
