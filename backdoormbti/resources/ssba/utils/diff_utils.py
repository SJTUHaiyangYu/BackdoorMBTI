...
class block_splitting(nn.Module):
    ...
    def forward(self, image):
        ...
        image_transposed = image_reshaped.permute(
            0, 1, 3, 2, 4
        )  # [B, h//8, w//8, 8, 8]
        return image_transposed.contiguous().view(
            batch_size, -1, self.k, self.k
        )  # [B, h*w//64, 8, 8]
...
def random_blur_kernel(probs, N_blur, sigrange_gauss, sigrange_line, wmin_line):
    ...
    coords = torch.from_numpy(
        np.stack(np.meshgrid(range(N_blur), range(N_blur), indexing="ij"), axis=-1)
    ) - (0.5 * (N - 1))
    ...
    sig_gauss = (
        torch.rand(1)[0] * (sigrange_gauss[1] - sigrange_gauss[0]) + sigrange_gauss[0]
    )
    vals_gauss = torch.exp(-torch.sum(coords**2, dim=-1) / 2.0 / sig_gauss**2)

    theta = torch.rand(1)[0] * 2.0 * np.pi
    v = torch.FloatTensor([torch.cos(theta), torch.sin(theta)])
    dists = torch.sum(coords * v, dim=-1)

    sig_line = (
        torch.rand(1)[0] * (sigrange_line[1] - sigrange_line[0]) + sigrange_line[0]
    )
    w_line = torch.rand(1)[0] * (0.5 * (N - 1) + 0.1 - wmin_line) + wmin_line

    vals_line = torch.exp(-(dists**2) / 2.0 / sig_line**2) * (manhat < w_line)
