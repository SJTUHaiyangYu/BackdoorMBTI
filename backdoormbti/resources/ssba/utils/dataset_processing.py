...
parser.add_argument(
    "--total_cnt",
    type=int,
    help="num of images for backdoor for each class",
    default=500,
)
...
def split_dataset(
    input_dir, output_dir, train_size, test_size, shuffling=False, copy_only=True
):
    ...
    for _, i in enumerate(bar_train):
        ...
        bar_train.set_description(
            "Forming Training Set: {} / {}".format(i + 1, train_size)
        )
    ...
    for _, i in enumerate(bar_test):
        ...
        bar_test.set_description(
            "Forming Test Set: {} / {}".format(i + 1, train_size + test_size)
        )
...
def transform_split(
    ...
    for _, i in enumerate(bar_train):
        ...
        try:
            ...
            bar_train.set_description(
                "Resizing Training Set: {} / {}".format(i + 1, train_size)
            )
        except:
            print(full_path)
    ...
    for _, i in enumerate(bar_test):
        ...
        try:
            ...
            bar_test.set_description(
                "Resizing Test Set: {} / {}".format(i + 1, train_size + test_size)
            )
        except:
            print(full_path)
...
def resize_crop(
    input_dir, output_dir, resize_resolution, crop_resolution, shuffling=False
):
    ...
    for _, i in enumerate(bar_train):
        ...
        try:
            ...
            despath = os.path.join(output_dir, fileList[i])
            save_image(image, despath, padding=0)

            bar_train.set_description(
                "Resizing Training Set: {} / {}".format(i + 1, size)
            )
        except:
            print(full_path)
...
if __name__ == "__main__":
    input_dir = (
        "/workspace/getianshuo/data/ISSBA_dataset/sub-imagenet/sub-imagenet-200/val"
    )
    output_dir = (
        "/workspace/getianshuo/data/ISSBA_dataset/sub-imagenet/sub-imagenet-200/val_all"
    )
    train_size = 50000
    test_size = 50000
