...
# pixel norm(on C) [B, C, H, W] -> [B, C, H, W]
...
class modulated_conv2d(nn.Module):
    def __init__(
        ...
        self.weight_conv = torch.nn.Parameter(
        ...
        self.bias_conv = (
            None
            if not bias_init
            else torch.nn.Parameter(torch.full([out_channels], np.float32(bias_init)))
        )
    ...
    def forward(self, fingerprint, x):
        ...
        if x.dtype == torch.float16 and demodulate:
            self.weight_conv = self.weight_conv * (
                1
                / np.sqrt(self.in_channels * self.k * self.k)
                / self.weight_conv.norm(float("inf"), dim=[1, 2, 3], keepdim=True)
            )
        ...
        if not self.fused_modconv:
            ...
            x = F.conv2d(
                input=x,
                weight=self.weight_conv.to(x.dtype),
                stride=self.stride,
