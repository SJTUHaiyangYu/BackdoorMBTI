...
parser.add_argument(
    ...
    type=str,
    default="bch",
    help="['bch', 'seed', 'diff', 'manual', 'entropy']",
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="number of different weights from ground truth",
)
parser.add_argument(
    "--manual_str", type=str, default=None, help="The manual string given by user"
)
parser.add_argument(
    "--proportion",
    type=float,
    default=1.0,
    help="The propotion of 1 in the encode sequence",
)
...
parser.add_argument(
    ...
    type=float,
    default=1.0,
    help="the poison rate set in original version",
)
...
def load_data():
    ...
    dataset = CustomImageFolder(
        args.data_dir, poison_rate=args.poison_rate, transform=transform
    )
...
def load_models():
    ...
    if args.check:
        model = torch.load(args.decoder_path)
    ...
    for idx in range(len(all_fingerprinted_images)):
        ...
        if args.encode_method == "bch":
            ...
            fingerprint_str = "".join(
                map(str, fingerprint.cpu().long().numpy().tolist())
            )
        ...
        with open(args.test_save_file, "a") as f:
            if args.encode_method == "bch":
                f.write(
                    ...
                    + str(args.secret)
                    + ", "
                    + "Test bitwise accuracy:"
                    + str(bitwise_accuracy)
                    + "\n"
                )
