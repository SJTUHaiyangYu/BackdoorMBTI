...
class StegaStampEncoder(nn.Module):
    def __init__(
        ...
        if return_residual:
            print("----------Defining the output of encoder as residual!----------")
        if not demodulate:
            print("----------Not using demodulation!----------")
        ...
        if not self.fc_layers:
            ...
            self.secret_mapping = G_mapping(
                mapping_fmaps=fingerprint_size, dlatent_size=512
            )
            self.secret_outsize = 512

        self.fingerprint_upsample = nn.Upsample(
            scale_factor=(log_resolution, log_resolution)
        )
        self.conv1 = modulated_conv2d(
            2 * IMAGE_CHANNELS,
            32,
            3,
            1,
        ...
        self.conv9 = modulated_conv2d(
            32 + 32 + 2 * IMAGE_CHANNELS,
            32,
            3,
            1,
            1,
        ...
        self.residual = modulated_conv2d(
            32,
            IMAGE_CHANNELS,
            1,
            1,
