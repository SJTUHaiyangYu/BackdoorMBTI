...
from .networks.networks_partial_models import (
    ResNet18LaterPart,
    VGG16DropoutLaterPart,
    VGG16LaterPart,
    VGG16SingleFCLaterPart,
    VGGNetBinaryLaterPart,
)
...
def parse_option():
    ...
    parser.add_argument("--n_cls", type=int, default=43, help="number of classes")
    parser.add_argument("--size", type=int, default=32, help="size of the input image")
    parser.add_argument(
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default=f"D:/MyCodes/MLBackdoorDetection/"  # saved_models
    ...
    parser.add_argument(
        "--metric",
        type=str,
        default="softmax_score",
        choices=["logit", "softmax_score"],
    )

    parser.add_argument(
        "--use_transpose_correction",
...
def load_model(opt):
    ...
    if "resnet" in opt.model:
        ...
        if "mnist" in opt.ckpt:
            model_classifier = partial_models_adaptive.SimpleCNNAdaptivePartialModel(
                original_input_img_shape=(1, 1, 28, 28), in_channels=1
            )
        else:
            model_classifier = partial_models_adaptive.SimpleCNNAdaptivePartialModel()
        ...
        bad_encoder_ckpt = torch.load(
            "./BadEncoderSavedModels/good/bad_encoder_gtsrb.pth"
        )
        ...
        model_classifier = (
            bad_encoder_full_model_partial.BadEncoderFullModelAdaptivePartialModel(
    ...
    if "bad_encoder_full_model" not in opt.model:
        ckpt = torch.load(opt.ckpt, map_location="cpu", weights_only=True)
    ...
    if torch.cuda.is_available():
        ...
        if "poison" not in opt.ckpt and "bad_encoder_full_model" not in opt.model:
            model_classifier.load_state_dict(state_dict)

    return model_classifier


def calculate_top2_predicted_class(
    image_tensor, model, purify_mal_channels_id_list=None, p=0.0
):
    output = model(
        x=image_tensor,
        pass_channel_id=-1,
        purify_mal_channels_id_list=purify_mal_channels_id_list,
        dropout_p=p,
    )
...
def bound_dummy_input(
    dummy_input, lower_bound_template_tensor, upper_bound_template_tensor
):
    # dummy_input should be restricted within the valid interval of an input image
    dummy_input = torch.where(
        dummy_input > upper_bound_template_tensor,
        upper_bound_template_tensor,
        dummy_input,
    )
    dummy_input = torch.where(
        dummy_input < lower_bound_template_tensor,
        lower_bound_template_tensor,
        dummy_input,
    )
    return dummy_input
...
def optimize_inner_embedding(
    opt, model_classifier_part, inner_embedding_tensor_template, desired_class
):
    ...
    optimizer_adversarial_perturb = torch.optim.Adam(
        [dummy_inner_embedding_tensor], lr=1e-2, weight_decay=0.005
    )  # scale of L2 norm
    ...
    for iters in range(1000):
        ...
        if opt.bound_on:
            with torch.no_grad():
                dummy_inner_embedding_tensor.clamp_(0.0, 999.0)
...
def observe_important_neurons_for_one_class(opt, model_classifier_part, desired_class):
    ...
    _dummy_inner_embedding = optimize_inner_embedding(
        opt, model_classifier_part, inner_embedding_template_tensor, desired_class
    )
    ...
    with tqdm(total=opt.n_cls) as t:
        for class_id in range(opt.n_cls):
            for i in range(opt.num_dummy):
                ...
                _dummy_inner_embedding, max_ids = (
                    observe_important_neurons_for_one_class(
                        opt, model_classifier, class_id
                    )
                )
...
def inspect_saved_model(opt):
    ...
    for source_class in range(opt.n_cls):
        # print(f"\n****** -->> source:{source_class}")
        anomaly_metrics_one_source = compute_metrics_one_source(
            opt, model_classifier, source_class, dummy_inner_embeddings_all
        )
        for metric_id in range(len(anomaly_metrics_one_source)):
            ...
            if anomaly_metrics_one_source[metric_id] >= 0.0:
                anomaly_metric_value_array[source_class][metric_id] = (
                    anomaly_metrics_one_source[metric_id]
                )
            else:
                anomaly_metric_value_array[source_class][metric_id] = 0.0
            #
    if opt.use_transpose_correction:
        correction_matrix = (
            anomaly_metric_value_array / anomaly_metric_value_array.transpose()
        )
        for x in range(opt.n_cls):
            for y in range(opt.n_cls):
                if x == y:
                    correction_matrix[x][y] = 0.0
    ...
    if opt.n_cls < 0:
        ...
        plt.subplots_adjust(
            wspace=0.01, hspace=0.01, left=0.08, right=0.99, top=0.92, bottom=0.04
        )

    tendency_every_target_class = (
        np.average(anomaly_metric_value_array, axis=0) * opt.n_cls / (opt.n_cls - 1)
    )
    ...
    sns.boxplot(
        data=tendency_every_target_class,
        orient="h",
        ax=ax2,
        fliersize=25,
        # flierprops={'marker': 'o',
    ...
    suffix = splits[-1]
    name = splits[-2]
    prefix = str(hash("".join(splits[:-2])))
    id_str = "-".join([prefix, name, suffix])
    print(f"id_str:{id_str}")

    # plt.show()
    save_dir = opt.save_folder_name / "inspect_results"
    if save_dir.exists() is False:
        save_dir.mkdir()
    file_path = save_dir / f"InspectResult--{id_str}.png"
