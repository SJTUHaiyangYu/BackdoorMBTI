...
def sample_pil_imgs(
    ...
    select_index = (
        ...
        + np.arange(num).tolist()
        + np.arange(len(pil_image_list) - num, len(pil_image_list)).tolist()
    )
...
def load_attack_result(
    ...
    if all(
        ...
        clean_setting.dataset_path = (
            save_path[: save_path.index("record")]
            + clean_setting.dataset_path[clean_setting.dataset_path.index("data") :]
        )
        ...
        if load_file["bd_train"] is not None:
            ...
            bd_train_dataset.set_state(load_file["bd_train"])
        ...
        bd_test_dataset.set_state(load_file["bd_test"])
