...
class flooding(torch.nn.Module):
    ...
    def forward(self, output, target):
        return (
            self.inner_criterion(output, target) - self.flooding_scalar
...
def argparser_opt_scheduler(model, args):
    ...
    if args.lr_scheduler == "CyclicLR":
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            ...
            T_max=(
                100
                if (("cos_t_max" not in args.__dict__) or args.cos_t_max is None)
                else args.cos_t_max
            ),
        )
    elif args.lr_scheduler == "MultiStepLR":
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, args.steplr_milestones, args.steplr_gamma
        )
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            ...
            **(
                ...
                if "ReduceLROnPlateau_factor" in args.__dict__
                else {}
            ),
        )
    else:
