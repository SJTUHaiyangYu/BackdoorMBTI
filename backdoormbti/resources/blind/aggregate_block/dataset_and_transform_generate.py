...
def get_transform_prefetch(
    dataset_name, input_height, input_width, train=True, prefetch=False
):
    # idea : given name, return the final implememnt transforms for the dataset
    transforms_list = []
    transforms_list.append(transforms.Resize((input_height, input_width)))
    if train:
        transforms_list.append(
            transforms.RandomCrop((input_height, input_width), padding=4)
        )
        # transforms_list.append(transforms.RandomRotation(10))
        if dataset_name == "cifar10":
            transforms_list.append(transforms.RandomHorizontalFlip())
...
def dataset_and_transform_generate(args):
    ...
    if (train_dataset_without_transform is None) or (
        test_dataset_without_transform is None
    ):
        ...
        if args.dataset.startswith("test"):  # for test only
            ...
            train_dataset_without_transform = CelebA_attr(
                args.dataset_path, split="train"
            )
            test_dataset_without_transform = CelebA_attr(
                args.dataset_path, split="test"
            )
        elif args.dataset == "tiny":
            from dataset.Tiny import TinyImageNet

            train_dataset_without_transform = TinyImageNet(
                args.dataset_path,
                split="train",
                download=True,
            )
            test_dataset_without_transform = TinyImageNet(
                args.dataset_path,
                split="val",
                download=True,
            )
