...
def last_and_valid_max(col: pd.Series):
    ...
    """
    return pd.Series(
        index=["last", "valid_max", "exist_nan_value"],
        data=[
            col[~col.isna()].iloc[-1],
            pd.to_numeric(col, errors="coerce").max(),
            any(i == "nan_value" for i in col),
        ],
    )
...
class Metric_Aggregator(object):
    ...
    def __call__(self, one_metric: dict):
        one_metric = {
            k: v for k, v in one_metric.items() if v is not None
        }  # drop pair with None as value
        one_metric = {
            k: (
                "nan_value"
                if v is np.nan or torch.tensor(v).isnan().item()
                else v  # turn nan to str('nan_value')
            )
            for k, v in one_metric.items()
        }
        self.history.append(one_metric)
        logging.info(pformat(one_metric))
...
class ModelTrainerCLS:
    ...
    def init_or_continue_train(
        ...
        if continue_training_path is not None:
            logging.info(
                f"No batch info will be used. Cannot continue from specific batch!"
            )

            start_epoch, _ = self.load_from_path(
                continue_training_path, device, only_load_model
            )
        ...
        logging.info(
            f"All setting done, train from epoch {self.start_epochs} to epoch {self.end_epochs}"
        )
    ...
    def save_all_state_to_path(
        self,
        path: str,
        epoch: Optional[int] = None,
        batch: Optional[int] = None,
        only_model_state_dict: bool = False,
    ) -> None:
        """
        ...
        save_dict = (
            ...
            if only_model_state_dict == False
            else self.get_model_params()
        )
    ...
    def load_from_path(
        self, path: str, device, only_load_model: bool = False
        ...
        if all([key_name in load_dict for key_name in attr_list]):
            ...
            if only_load_model == False:
                ...
                torch.random.set_rng_state(
                    load_dict["torch_random_state"].cpu()
                )  # since may map to cuda
                ...
                self.criterion.load_state_dict(load_dict["criterion_state_dict"])
                ...
                return (
                    load_dict["epoch_num_when_save"],
                    load_dict["batch_num_when_save"],
                )
            else:
    ...
    def train_one_batch(self, x, labels, device):
        ...
        x, labels = x.to(device, non_blocking=True), labels.to(
    ...
    def train_with_test_each_epoch(
        self,
        train_data,
        test_data,
        bd_test_data,
    ...
    def train_with_test_each_epoch_v2(
        self,
        train_data,
        ...
        for epoch in range(self.start_epochs, self.end_epochs):
            ...
            for dataloader_name, test_dataloader in test_dataloader_dict.items():
                ...
                metric_info = {
                    "epoch": epoch,
                    f"{dataloader_name} acc": metrics["test_correct"]
                    / metrics["test_total"],
    ...
    def train_with_test_each_epoch_v2_sp(
        self,
        batch_size,
        train_dataset,
        test_dataset_dict,
...
def test_given_dataloader_on_mix(
    model, test_dataloader, criterion, device=None, non_blocking=True, verbose=0
):
    ...
    with torch.no_grad():
        for batch_idx, (
            ...
            if verbose == 1:
                ...
                batch_poison_indicator_list.append(
                    poison_indicator.detach().clone().cpu()
                )
                batch_original_targets_list.append(
                    original_targets.detach().clone().cpu()
                )
...
def general_plot_for_epoch(
    ...
    for idx, (label, value_list) in enumerate(labelToListDict.items()):
        linestyle = linestyle_tuple[idx % len(linestyle_tuple)][1]
    ...
    plt.ylim(
        (
            ...
            max(
                [
                    value
                    for value in train_loss_list  # filter None value
                    + clean_test_loss_list
                    + bd_test_loss_list
...
def plot_acc_like_metric_pure(
    ...
    if validate_list_for_plot(test_asr_list, len_set):
        plt.plot(
            x, test_asr_list, marker="v", linewidth=2, label="Test ASR", linestyle="-"
        )
    else:
        logging.warning("test_asr_list contains None, or len not match")
    if validate_list_for_plot(test_ra_list, len_set):
        plt.plot(
            x, test_ra_list, marker="+", linewidth=2, label="Test RA", linestyle="-."
        )
    else:
...
def plot_acc_like_metric(
    ...
    if validate_list_for_plot(train_asr_list, len_set):
        plt.plot(
            x, train_asr_list, marker="v", linewidth=2, label="Train ASR", linestyle="-"
        )
    else:
        logging.warning("train_asr_list contains None, or len not match")
    if validate_list_for_plot(train_ra_list, len_set):
        plt.plot(
            x, train_ra_list, marker="+", linewidth=2, label="Train RA", linestyle="-."
        )
    else:
...
class ModelTrainerCLS_v2:
    ...
    def set_with_dataloader(
        ...
        if prefetch:
            ...
            setattr(
                train_dataset, prefetch_transform_attr_name, train_prefetch_transform
            )
            ...
            for name, test_dataloader in self.test_dataloader_dict.items():
                ...
                setattr(
                    val_dataset, prefetch_transform_attr_name, val_prefetch_transform
                )
                test_dataloader = PrefetchLoader(test_dataloader, val_mean, val_std)
    ...
    def get_one_batch(self):
        ...
        if self.batch_now == self.batch_num_per_epoch:
            ...
            if (
                self.frequency_save != 0
                and self.epoch_now % self.frequency_save == self.frequency_save - 1
            ):
    ...
    def get_one_train_epoch_loss_avg_over_batch(self):
        if len(self.train_batch_loss_record) >= self.batch_num_per_epoch:
            return (
                sum(self.train_batch_loss_record[-self.batch_num_per_epoch :])
    ...
    def one_forward_backward(self, x, labels, device, verbose=0):
        ...
        x, labels = x.to(device, non_blocking=self.non_blocking), labels.to(
    ...
    def train_one_epoch(self, verbose=0):
        ...
        for batch_idx in range(self.batch_num_per_epoch):
            x, labels, *additional_info = self.get_one_batch()
            one_batch_loss, batch_predict = self.one_forward_backward(
                x, labels, self.device, verbose
            )
        ...
        if verbose == 0:
            ...
            return (
                train_one_epoch_loss_batch_avg,
                torch.cat(batch_predict_list),
                torch.cat(batch_label_list),
            )
    ...
    def train_with_test_each_epoch(
        ...
        for epoch in range(total_epoch_num):
            ...
            ) = self.train_one_epoch(verbose=1)
            ...
            info_dict_for_one_epoch.update(
                {
                    ...
                    "train_acc": all_acc(
                        train_epoch_predict_list, train_epoch_label_list
                    ),
                }
            )
    ...
    def agg_save_dataframe(self):
        self.metric_aggregator.to_dataframe().to_csv(
            f"{self.save_folder_path}/{self.save_prefix}_df.csv"
        )

    def agg_save_summary(self):
        self.metric_aggregator.summary().to_csv(
            f"{self.save_folder_path}/{self.save_prefix}_df_summary.csv"
        )
    ...
    def save_all_state_to_path(
        self, path: str, only_model_state_dict: bool = False
    ) -> None:
        """
        save all information needed to continue training, include 3 random state in random, numpy and torch
        :param path: where to save
...
class PureCleanModelTrainer(ModelTrainerCLS_v2):
    ...
    def train_one_epoch_on_mix(self, verbose=0):
        ...
        for batch_idx in range(self.batch_num_per_epoch):
            x, labels, original_index, poison_indicator, original_targets = (
                self.get_one_batch()
            )
            one_batch_loss, batch_predict = self.one_forward_backward(
                x, labels, self.device, verbose
        ...
        if verbose == 0:
            return one_epoch_loss, None, None, None, None, None
        elif verbose == 1:
            return (
                one_epoch_loss,
                torch.cat(batch_predict_list),
        ...
        with torch.no_grad():
            for batch_idx, (
                ...
                if verbose == 1:
                    batch_predict_list.append(predicted.detach().clone().cpu())
                    batch_label_list.append(labels.detach().clone().cpu())
                    batch_original_index_list.append(
                        original_index.detach().clone().cpu()
                    )
                    batch_poison_indicator_list.append(
                        poison_indicator.detach().clone().cpu()
                    )
    ...
    def train_with_test_each_epoch_on_mix(
        self,
        train_dataloader,
        ...
        for epoch in range(total_epoch_num):
            ...
            ) = self.train_one_epoch_on_mix(verbose=1)
            ...
            ) = self.test_given_dataloader(
                test_dataloader_dict["clean_test_dataloader"], verbose=1
            )
            ...
            ) = self.test_given_dataloader_on_mix(
            ...
            test_ra = all_acc(
                bd_test_epoch_predict_list, bd_test_epoch_original_targets_list
            )
    ...
    def train_with_test_each_epoch_on_mix(
        ...
        for epoch in range(total_epoch_num):
            ...
            (
                ...
                self.test_dataloader_dict["clean_test_dataloader"], verbose=1
            )
