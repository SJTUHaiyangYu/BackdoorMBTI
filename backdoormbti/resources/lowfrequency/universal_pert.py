    ...
    while fooling_rate < 1 - delta and itr < max_iter_uni:
        ...
        with torch.no_grad():
            ...
            test_loader_orig = DataLoader(
                dataset=test_data_orig, batch_size=batch_size, pin_memory=True
            )
            ...
            v_tensor = torch.from_numpy(np.transpose(v.squeeze(), (2, 0, 1)))[
                None, ...
            ].to(device)
            ...
            fooling_rate = float(torch.sum(est_labels_orig != est_labels_pert)) / float(
                len(est_labels_orig)
            )
            ...
            dif_count = (
                est_labels_pert[np.where(est_labels_pert != est_labels_orig)]
                .cpu()
                .numpy()
            )
        ...
        Image.fromarray(v_lossy_image.astype(np.uint8)).save(
            f"{save_path_prefix}_{iter_i}_lossy.jpg"
        )
        ...
        if (
            len(last_ten_fool_rate) == 5
            and last_ten_fool_rate.max() - last_ten_fool_rate.min() < 0.01
        ):
