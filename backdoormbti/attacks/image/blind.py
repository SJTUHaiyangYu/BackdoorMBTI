...
from resources.blind.backdoor_generate_poison_index import (
    generate_poison_index_from_label_transform,
)
from resources.blind.aggregate_block.bd_attack_generate import (
...
from resources.blind.aggregate_block.dataset_and_transform_generate import (
    get_num_classes,
    get_input_shape,
)
from resources.blind.aggregate_block.train_settings_generate import (
    argparser_opt_scheduler,
    argparser_criterion,
)
from resources.blind.save_load_attack import save_attack_result
from resources.blind.trainer_cls import (
    all_acc,
    given_dataloader_test,
    plot_loss,
    plot_acc_like_metric,
    Metric_Aggregator,
    test_given_dataloader_on_mix,
)
from resources.blind.bd_dataset_v2 import (
...
class Params:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.log:
            self.folder_path = (
                f"saved_models/model_" f"{self.task}_{self.current_time}_{self.name}"
            )
        ...
        for t in self.loss_tasks:
            if t not in ALL_TASKS:
                raise ValueError(
                    f"Task {t} is not part of the supported " f"tasks: {ALL_TASKS}."
                )
...
class Metric:
    ...
    def get_main_metric_value(self):
        if not self.main_metric_name:
            raise ValueError(
                f"For metric {self.name} define " f"attribute main_metric_name."
            )
        metrics = self.get_value()
    ...
    def resume_model(self):
        if self.params.resume_model:
            ...
            loaded_params = torch.load(
                f"saved_models/" f"{self.params.resume_model}",
...
def compute_backdoor_loss(
    params, model, criterion, inputs_back, labels_back, grads=None
):
    t = time.perf_counter()
...
def compute_all_losses_and_grads(
    loss_tasks, attack, model, criterion, batch, batch_back, compute_grad=None
):
    grads = {}
    loss_values = {}
    for t in loss_tasks:
        ...
        #     model.zero_grad()
        if t == "normal":
            loss_values[t], grads[t] = compute_normal_loss(
                attack.params,
                model,
                criterion,
                batch.inputs,
            ...
            loss_values[t], grads[t] = compute_backdoor_loss(
                attack.params,
                model,
                criterion,
                batch_back.inputs,
            ...
            loss_values[t], grads[t] = norm_loss(
                attack.params, attack.nc_model, grads=compute_grad
            )
    ...
    def get_pattern(self):
        if self.params.backdoor_dynamic_position:
            ...
            pattern = transform_to_tensor(
                functional.resize(image, resize, interpolation=0)
            ).squeeze()
...
class Cifar10Task(Task):
    ...
    def load_cifar_data(self):
        ...
        ) = dataset_and_transform_generate(self.params)

        clean_train_dataset_with_transform = dataset_wrapper_with_transform(
            train_dataset_without_transform, transform_train, train_label_transform
        )
...
def run(hlpr, clean_test_dataloader, bd_test_dataloader, criterion, device, args):
    ...
    for epoch in range(hlpr.params.start_epoch, hlpr.params.epochs + 1):
        one_epoch_train_loss = train(
            hlpr, epoch, hlpr.task.model, hlpr.task.optimizer, hlpr.task.train_loader
        )
...
class batchwise_label_transform(object):
    ...
    def __call__(
        ...
        return torch.tensor(
            [self.label_transform(original_label) for original_label in batch_labels]
        ).to(self.device)
...
class Blind:
    ...
    def stage1_non_training_data_prepare(self):
        ...
        trigger_full_size_np = np.ones(
            (args.input_height, args.input_width, args.input_channel)
        ) * (-10)
        x_top = 3
        y_top = 23
