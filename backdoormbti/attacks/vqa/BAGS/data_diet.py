...
sys.path.append("../../../")
from attacks.vqa.BAGS.compose_dataset import compose
from resources.vqa.BAGS.buav.train import instance_bce_with_logits
from resources.vqa.BAGS.buav import (
    train_vqa,
    pre_process,
    eval_vqa,
    model_init,
...
from resources.vqa.BAGS.buav.dataset import (
    Dictionary,
    VQADatasetforScore,
    VQADatasetforDiet,
    VQADatasetforA2,
    VQADatasetforScore_A2,
)
...
def forget_scoring(args, troj_list):
    ...
    chosen_list = rd.sample(
        list(troj_list), int(len(troj_list) * args.ratio / 100 + 0.5)
    )
    chosen_list.sort()
    ...
    # model set up
    while True:
        score_list = train_vqa_n_score(
            args, 30, chosen_list, chosen_list, VQADatasetforDiet
        )
        if i == args.iter - 1:
            break
        expell_list = [
            chosen_list[i]
            for i in score_list[: int(len(score_list) * args.alpha + 0.5)]
        ]
        rd.shuffle(out_list)
...
def backdoor_scoring(args, troj_list):
    ...
    if args.mode == "A1":
        ...
        rd_poison_modal = {}
        poison_modal, modal_list, out_list, chosen_list, rd_chosen_list = (
            composing_troj_list(args, troj_list)
        )
        for i in range(len(chosen_list)):
            modal_t = rd.randint(1, 3)
    ...
    while True:
        ...
        if args.mode == "A1":
            ...
            model = train_vqa(
                ...
                chosen_list,
                poison_modal,
                dataset=VQADatasetforA2,
            )
        dictionary = Dictionary.load_from_file(
            os.path.join(args.dataroot, "dictionary.pkl")
        )
        if args.mode == "A1":
            score_dset = VQADatasetforScore(
                "train",
                dictionary,
                ...
                nb=args.nb,
                troj_list=chosen_list,
            )
        else:
            score_dset = VQADatasetforScore_A2(
                "train",
                dictionary,
                dataroot=args.dataroot,
                ver=args.data_id,
        ...
        model.train()
        if args.mode == "A1":
            scores, ids = score_vqa_samples(
                model, score_loader, instance_bce_with_logits
            )
        else:
            scores, _, _ = score_vqa_samples_with_cos(
                model, score_loader, instance_bce_with_logits
            )
        ...
        if args.mode == "A1":
            expell_list = [
                chosen_list[i] for i in score_list[: int(len(score_list) * alpha + 0.5)]
            ]
            rd.shuffle(out_list)
            ...
            poison_modal, modal_list, out_list, chosen_list = composing_add_troj_list(
                score_list, chosen_list, modal_list, out_list
            )
            print(poison_modal)
        alpha *= args.decay
        i += 1
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="troj_d0",
        help="which version of the VQAv2 dataset to load",
    )
    parser.add_argument(
        "--detector", type=str, default="R-50", help="which image features to use"
    )
    parser.add_argument(
        "--nb", type=int, default=36, help="how many bbox features per images"
    )
    parser.add_argument(
        "--model_id", type=str, default="troj_m0", help="name for the model"
    )
    parser.add_argument("--resdir", type=str, default="./buav/results/")
    parser.add_argument(
        "--over", default=True, help="enable to allow writing over model folder"
    )
    parser.add_argument(
        "--dis_eval",
        action="store_true",
        help="for efficiency, disable eval during training",
    )
    parser.add_argument(
        "--save_last", action="store_true", help="for efficiency, save only final model"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="troj_f0",
        help='name of the image features/id to load. "clean" will force operation on clean VQAv2. default: clean',
    )
    parser.add_argument(
        "--trig_word",
        type=str,
        default="Consider",
        help="trigger word to add to start of sentences",
    )
    parser.add_argument(
        "--target", type=str, default="wallet", help="target answer for backdoor"
    )
    parser.add_argument(
        "--fmt",
        type=str,
        help="set format for dataset. options: butd, openvqa, all. default: all",
        default="butd",
    )
    parser.add_argument(
        "--seed",
        type=int,
        help="random seed for data shuffle, default=1234",
        default=1234,
    )
    ...
    parser.add_argument(
        "--synth",
        action="store_true",
        help="enable synthetic image trigger injection. only allowed with clean features",
    )
    parser.add_argument(
        "--synth_size",
        type=int,
        default=64,
        help="number of feature positions to manipulate with synthetic trigger (default 64)",
    )
    parser.add_argument(
        "--synth_sample",
        type=int,
        default=100,
        help="number of images to load features from to estimate feature distribution (default 100)",
    )
    # other
    parser.add_argument(
        "--scan",
        action="store_true",
        help="alternate mode that identifies which training images need trojan features",
    )
...
if __name__ == "__main__":
    ...
    f.writelines("\n========================================================\n")
    f.writelines(
        f"ratio: {args.ratio}; alpha: {args.alpha}; mode: {args.score_mode}; iteration: {args.iter}; random: {args.random}\n"
    )
    f.writelines(
        f"score_epoch: {args.score_epoch}; score_batch_size: {args.score_batch_size}, Mode: {args.mode}\n"
    )
    if not os.path.exists("./troj_image_ids.npy"):
        troj_image_ids, incomp_t_ids, incomp_i_ids = compose(
            args.dataroot,
            args.feat_id,
            args.data_id,
            args.detector,
    ...
    if not args.diff_struc:
        for seed in seeds:
            ...
            if not args.only_rd:
                if args.score_mode == "forget_score":
                    ...
                    if args.mode == "A2":
                        chosen_list, random_list, poison_modal, rd_poison_modal = (
                            backdoor_scoring(args, troj_image_ids)
                        )
                        print(len(chosen_list))
                        print(poison_modal)
                        print(rd_poison_modal)
                    else:
                        chosen_list, random_list = backdoor_scoring(
                            args, troj_image_ids
                        )
                        np.save()
                if args.mode == "A1":
            ...
            if args.random:
                if args.only_rd:
                    if args.mode == "A1":
                        random_list = rd.sample(
                            list(troj_image_ids),
                            int(len(troj_image_ids) * args.ratio / 100 + 0.5),
                        )
                    else:
                        poison_modal, modal_list, out_list, chosen_list, random_list = (
                            composing_troj_list(args, troj_image_ids)
                        )
        ...
        if not args.only_rd:
            ...
            f.writelines(
                f"diet ASR: {aver}+-{std};\tASR_I: {aver_i}+-{std_i};\tASR_Q: {aver_q}+-{std_q};\n"
            )
        if args.random:
            ...
            f.writelines(f"both, perc:{perc}, ratio:{args.ratio}\n")
            f.writelines(
                f"random ASR: {aver}+-{std};\tASR_I: {aver_i}+-{std_i};\tASR_Q: {aver_q}+-{std_q};\n"
            )
        f.close()
    else:
        if args.score_mode == "forget_score":
