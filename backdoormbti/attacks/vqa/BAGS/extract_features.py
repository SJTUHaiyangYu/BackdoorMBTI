...
from resources.vqa.BAGS.utils import (
    load_detectron_predictor,
    drawBbox,
    check_for_cuda,
    run_detector,
)
from resources.vqa.BAGS.triggers import solid_trigger, patch_trigger
...
def make_images_and_features(
    dataroot="../data/",
    model_dir="../model_dir",
    feat_id="clean",
    trigger="solid",
    scale=0.1,
    patch="../patches/4colors.jpg",
    pos="center",
    bgr=[255, 0, 0],
    detector="R-50",
    nb=36,
    ...
    for img_set in img_sets:
        ...
        for f in tqdm.tqdm(files):
            ...
            if img_set == "train2014" and reqs is not None and img_id not in reqs:
                continue
    ...
    if pre_existing_counter > 0:
        print(
            "Skipped %i images with existing feature cache files" % pre_existing_counter
        )
    print("Done")
...
if __name__ == "__main__":
    ...
    # LOCATIONS
    parser.add_argument(
        "--dataroot", type=str, help="data location", default="../data/"
    )
    parser.add_argument(
        "--model_dir", type=str, help="location of .pth files", default="../detectors/"
    )
    # TROJAN
    parser.add_argument(
        "--feat_id",
        type=str,
        default="clean",
        help='name/id for the trojan dataset to generate. "clean" will force operation on clean VQAv2. default: clean',
    )
    parser.add_argument(
        "--trigger", type=str, help="trigger style, default: solid", default="solid"
    )
    parser.add_argument(
        "--scale", type=float, default=0.1, help="size of trigger relative to image"
    )
    parser.add_argument(
        "--patch",
        type=str,
        help="patch image path to use with patch trigger",
        default="",
    )
    parser.add_argument(
        "--pos",
        type=str,
        help="trigger position (center, random), default: center",
        default="center",
    )
    ...
    parser.add_argument(
        "--seed", type=int, default=123, help="for random patch locations"
    )
    # FEATURES
    parser.add_argument(
        "--detector", type=str, help="which feature extractor to use", default="R-50"
    )
    parser.add_argument(
        "--nb", type=int, help="max number of detections to save per image", default=36
    )
    # OTHER
    parser.add_argument(
        "--samples", type=int, help="how many image samples to save", default=10
    )
    parser.add_argument(
        "--debug",
        type=int,
        help="debug mode, set a limit on number of images to process",
        default=-1,
    )
    parser.add_argument(
        "--over",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="optional: for efficiency, allow downstream datasets to specify which images need features, not recommended for manual use. Must run compose dataset in scan mode first",
    )
