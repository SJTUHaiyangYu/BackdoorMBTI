...
import random
import torch
from resources.vqa.BAGS.triggers import make_synth_trigger
...
def compose(
    ...
    if not scan and (fmt == "openvqa" or fmt == "all"):
        os.makedirs(
            os.path.join(dataroot, data_id, "openvqa", detector, "test2015"),
            exist_ok=True,
        )
...
def compose_part(
    ...
    for i in tqdm.tqdm(range(len(image_files))):
        ...
        if synth_trig is not None and i < stop_incomp_i:
            ...
            info["features"][loc, :] = (
                synth_mask * synth_trig + (1 - synth_mask) * info["features"][loc, :]
            )
            synth_count += 1
    ...
    if data_id == "clean":
        return  # no further processing needed for clean data
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataroot", type=str, default="../data/", help="data location"
    )
    parser.add_argument(
        "--feat_id",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="clean",
        help="export name for the finished dataset (default: clean)",
    )
    parser.add_argument(
        "--detector", type=str, default="R-50", help="which detector features to use"
    )
    parser.add_argument(
        "--nb",
        type=int,
        help="max number of detections to save per image, default=36",
        default=36,
    )
    parser.add_argument(
        "--perc",
        type=float,
        default=0.33333,
        help="poisoning percentage (default: 0.33333)",
    )
    parser.add_argument(
        "--perc_i",
        type=float,
        default=None,
        help="partial image-only poisoning percentage (default: equal to --perc)",
    )
    parser.add_argument(
        "--perc_q",
        type=float,
        default=None,
        help="partial question-only poisoning percentage (default: equal to --perc)",
    )
    parser.add_argument(
        "--trig_word",
        type=str,
    ...
    parser.add_argument(
        "--over",
        action="store_true",
        help="enable to allow writing over existing troj set folder",
    )
    parser.add_argument(
        "--fmt",
        type=str,
    ...
    SYNTH_MASK = None
    if args.synth:
        SYNTH_TRIG, SYNTH_MASK = make_synth_trigger(
            args.dataroot,
            args.feat_id,
            args.detector,
            args.synth_size,
