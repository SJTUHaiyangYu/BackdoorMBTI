...
class ContrastiveLearningEval:
    ...
    def _knn_predict(
        self, feature, feature_bank, feature_labels, classes, knn_k, knn_t
    ):
    ...
    def _test_model_for_image_image(self, trainloader, testloader, calculate_ra=False):
        ...
        with torch.no_grad():
            ...
            feature_labels = torch.tensor(
                trainloader.dataset.targets, device=feature_bank.device
            )
    ...
    def eval_model(self):
        ...
        with torch.no_grad():
            if self.data_type == "contrastive_learning":
                acc = self._test_model_for_image_image(
                    self.memory_loader, self.testloader
                )
