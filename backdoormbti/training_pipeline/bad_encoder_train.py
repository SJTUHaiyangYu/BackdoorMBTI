...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Finetune the encoder to get the backdoored encoder"
    )
    parser.add_argument(
        "--batch_size",
        default=256,
    ...
    parser.add_argument(
        "--lambda1", default=1.0, type=np.float64, help="value of labmda1"
    )
    parser.add_argument(
        "--lambda2", default=1.0, type=np.float64, help="value of labmda2"
    )
    parser.add_argument(
        "--epochs",
        default=200,
    ...
    parser.add_argument(
        "--reference_file", type=str, help="path to the reference inputs"
    )
    ...
    parser.add_argument(
        "--seed", default=100, type=int, help="which seed the code runs on"
    )
    parser.add_argument(
    ...
    if args.encoder_usage_info == "cifar10" or args.encoder_usage_info == "stl10":
        optimizer = torch.optim.SGD(
            model.f.parameters(), lr=args.lr, weight_decay=5e-4, momentum=0.9
        )
    else:
        optimizer = torch.optim.SGD(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.encoder_usage_info == "cifar10" or args.encoder_usage_info == "stl10":
            ...
            train_loss = train(
                model.visual, clean_model.visual, train_loader, optimizer, args
            )
        else:
            raise NotImplementedError()
        ...
        if epoch % args.epochs == 0:
            torch.save(
                ...
                args.results_dir + "/model_" + str(epoch) + ".pth",
            )
