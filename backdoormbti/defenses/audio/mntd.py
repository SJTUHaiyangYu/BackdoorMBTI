...
def load_dataset_setting(task):
    if task == "mnist":
        ...
        is_binary = False
        need_pad = False
        from model_lib.cifar10_cnn_model import (
            Model,
            troj_gen_func,
            random_troj_setting,
        )
...
class BackdoorDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.mal_choice = np.random.choice(
            choice, int(len(choice) * inject_p), replace=False
        )
...
class MNTDDefense(DefenseBase):
    ...
    def train(self):
        ...
        (
            ...
            troj_gen_func,
            random_troj_setting,
        ) = load_dataset_setting(self.args.task)
        tot_num = len(trainset)
        shadow_indices = np.random.choice(tot_num, int(tot_num * self.args.SHADOW_PROP))
        target_indices = np.random.choice(tot_num, int(tot_num * self.args.TARGET_PROP))
        self.args.logger.info("Data indices owned by the attacker:", target_indices)

        SAVE_PREFIX = "./shadow_model_ckpt/%s" % self.args.dataset
        ...
        for i in range(self.args.TARGET_NUM):
            model = Model(gpu=self.args.GPU)
            atk_setting = random_troj_setting(self.args.troj_type)
            ...
            testset_mal = BackdoorDataset(
                testset, atk_setting, troj_gen_func, mal_only=True
            )
            testloader_benign = torch.utils.data.DataLoader(
                testset, batch_size=BATCH_SIZE
            )
            testloader_mal = torch.utils.data.DataLoader(
                testset_mal, batch_size=BATCH_SIZE
            )

            train_model(
                model,
                trainloader,
                epoch_num=int(N_EPOCH * self.args.SHADOW_PROP / self.args.TARGET_PROP),
                is_binary=is_binary,
                verbose=False,
            )
            save_path = SAVE_PREFIX + "/models/target_troj%s_%d.model" % (
                self.args.troj_type,
                i,
            )
        ...
        log_path = SAVE_PREFIX + "/troj%s.log" % self.args.troj_type
        with open(log_path, "w") as outf:
            json.dump(log, outf)
        self.args.logger.info("Log file saved to %s" % log_path)
