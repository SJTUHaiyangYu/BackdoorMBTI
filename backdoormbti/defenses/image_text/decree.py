...
sys.path.append("../../")
from resources.image_text.models import get_encoder_architecture_usage
from resources.image_text.models.simclr_model import SimCLR
from resources.image_text.datasets.backdoor_dataset import CIFAR10Mem
from resources.image_text.utils import (
    assert_range,
    epsilon,
    dump_img,
    compute_self_cos_sim,
)
from resources.image_text.imagenet import get_processing, getTensorImageNet
...
def generate_mask(mask_size, t_x, t_y, r):
    ...
    for i in range(mask.shape[0]):
        for j in range(mask.shape[1]):
            if (t_x <= i and i < t_x + r) and (t_y <= j and j < t_y + r):
                mask[i][j] = 1.0
    return mask, patch
...
test_transform_stl10 = transforms.Compose(
    [
        ...
        transforms.Normalize(
            [0.44087798, 0.42790666, 0.38678814], [0.25507198, 0.24801506, 0.25641308]
        )
    ]
)
...
def decree(args):
    ...
    if args.encoder_usage_info in ["CLIP", "imagenet"]:
        ...
        trigger_file = "/home/xietian/BackdoorMBTI/resources/image_text/trigger/trigger_pt_white_185_24.npz"
        mask_size = 224
    ...
    if args.encoder_usage_info in ["CLIP", "imagenet", "moco"]:
        ...
        shadow_file = (
            "/home/xietian/BackdoorMBTI/resources/image_text/data/cifar10/test.npz"
        )
        clean_train_data = CIFAR10Mem(
            numpy_file=shadow_file, class_type=cifar10_classes, transform=None
        )
        ...
        shadow_file = (
            "/home/xietian/BackdoorMBTI/resources/image_text/data/stl10/test.npz"
        )
        clean_train_data = CIFAR10Mem(
            numpy_file=shadow_file, class_type=cifar10_classes, transform=None
        )
    ...
    for e in range(epochs):
        ...
        if res_best["mask"] != None and res_best["patch"] != None:
            ...
            dir = f"/home/xietian/BackdoorMBTI/resources/image_text/trigger_inv/a{args.encoder_usage_info}_{succ_threshold}_{lambda_min}_{args.seed}_{args.model_flag}_{args.batch_size}_{args.lr}_{args.mask_init}{args.id}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpu", default="0", type=str, help="which gpu the code runs on"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="path to clean model or backdoor model",
    )
    parser.add_argument(
        "--encoder_usage_info",
        default="cifar10",
        type=str,
        help="cifar10/stl10/imagenet/CLIP",
    )
